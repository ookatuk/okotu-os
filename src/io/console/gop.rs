...
pub const fn const_round(val: f64) -> f64 {
    (val + 0.5) as i64 as f64
}

const fn scale_f32_to_mask(intensity: f32, mask: u32) -> u32 {
    if mask == 0 {
        return 0;
    }
    ...
    let val = const_round((intensity * max_val as f32) as f64) as u32;
    val << shift
}
...
impl Color {
    ...
    pub const fn from_rgb(data: u32) -> Self {
    ...
    pub const fn get(
        &self,
        format: PixelFormat,
        mask: Option<PixelBitmask>,
    ) -> Option<result::Result<u32>> {
        ...
        Some(Ok(match format {
            PixelFormat::Rgb => {
                let r8 = const_round((r * 255f32) as f64) as u32;
                let g8 = const_round((g * 255f32) as f64) as u32;
                let b8 = const_round((b * 255f32) as f64) as u32;
                r8 | (g8 << 8) | (b8 << 16)
            }
            PixelFormat::Bgr => {
                let r8 = const_round((r * 255f32) as f64) as u32;
...
impl GopData {
    ...
    pub fn get_good_mode(&mut self) -> result::Result {
        ...
        without_interrupts(|| -> result::Result {
            ...
            for mode in gop.modes() {
                ...
                if let Some((level, _)) = ALLOW_RATIOS
                    .iter()
                    .enumerate()
                    .find(|&(_, &(rw, rh))| w * rh == h * rw)
                {
    ...
    pub unsafe fn draw_pixel(&mut self, x: usize, y: usize, color: Color) -> result::Result {
        let (w, h) = { (self.w.get(), self.h.get()) };

        if x < w && y < h {
            return unsafe { self.draw_pixel_unchecked(x, y, color) };
        }

        Error::new(ErrorType::InvalidData, Some("pixel out of bounds")).raise()
    }

    pub unsafe fn draw_pixel_unchecked(
        &mut self,
        x: usize,
        y: usize,
        color: Color,
