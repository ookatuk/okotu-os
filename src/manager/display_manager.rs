...
use crate::{
    BAR_HEIGHT, BAR_MARGIN, GUI_WAIT, MAIN_FONT, fonts, io, log_custom, log_debug, log_info, util,
};
...
const fn get_dynamic_rgb(tick: u16) -> u32 {
    const fn tri(mut x: u16) -> u8 {
        if x > 127 {
            x = 255 - x;
        }
        (x * 2) as u8
...
const fn blend_color(bg: u32, fg: u32, alpha: u8) -> u32 {
    if alpha == 0 {
        return bg;
    }
    if alpha == 255 {
...
impl DisplayManager {
        ...
        interrupts::without_interrupts(|| {
            ...
            if let Some(mut gop_lock) = me.gop_data.try_write() {
                if let Some(gop) = gop_lock.as_deref_mut() {
                    ...
                    let current_width =
                        (*me.do_parent.as_ref().read() * max_bar_width as f64) as usize;
                    ...
                    let (did_it_bar_color, do_it_bar_color) =
                        if me.last_log.lock().last_level == "last" {
                            ...
                            (Color::new(1.0, 1.0, 1.0), Color::from_rgb(0x444444))
                        };

                    unsafe {
                        // 2. プログレスバー描画
                        let _ =
                            gop.draw_rect(bar_x, bar_y, max_bar_width, BAR_HEIGHT, do_it_bar_color);
                        let _ = gop.draw_rect(
                            bar_x,
                            bar_y,
                            current_width,
                        ...
                        if let Some(font) = me.global_font.get().unwrap().read().as_ref() {
                            ...
                            let glyphs = {
                                ...
                                if lk.data == raw_msg {
                                    ...
                                    // 新しいログなのでラスタライズ実行
                                    let analyzed = fonts::analyze_text(
                                        font,
                            ...
                            for g in glyphs.iter() {
                                for row in 0..g.height {
                                    for col in 0..g.width {
                                        ...
                                        if let Some(Ok(raw)) =
                                            Color::from_rgb(blended_u32).get(gop.format, gop.mask)
                                        {
                                            let offset = (g.start_y + row as i32) as usize
                                                * gop.stride.get()
    ...
    pub fn start_load_grap(&self) -> result::Result {
        ...
        Error::try_raise(
            uefi::boot::set_timer(&event, TimerTrigger::Periodic(GUI_WAIT as u64)),
