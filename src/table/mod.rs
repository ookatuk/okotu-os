...
use crate::mem::paging::types::PHY_OFFSET;
...
impl TmpHandler {
    ...
    pub fn new() -> Self {
        let clock = without_interrupts(|| {
            TSC.read().clock_in_100ms
        });
        Self {
            clock_tsc: clock.unwrap().get() as u128
        }
    }
}
...
impl acpi::Handler for TmpHandler {
    unsafe fn map_physical_region<T>(&self, physical_address: usize, size: usize) -> PhysicalMapping<Self, T> {
        let virtual_address = PHY_OFFSET + physical_address;

        PhysicalMapping {
            physical_start: physical_address,
            virtual_start: NonNull::new(virtual_address as *mut T).expect("Virtual address is null"),
            region_length: size,
            mapped_length: size,
            handler: self.clone(),
        }
    ...
    fn nanos_since_boot(&self) -> u64 {
        let current_tsc = Tsc::now_clock_() as u128;
        let hz = self.clock_tsc * 10;
        (current_tsc * 1_000_000_000 / hz) as u64
    }

    fn stall(&self, microseconds: u64) {
        let start = Tsc::now_clock_();
        let ticks_to_wait = (microseconds as u128 * self.clock_tsc / 100_000) as u64;

        while Tsc::now_clock_() - start < ticks_to_wait {
            core::hint::spin_loop();
        }
    }
    ...
    fn acquire(&self, handle: Handle, timeout: u16) -> Result<(), AmlError> {
        ...
        let my_id = crate::cpu::utils::who_am_i();
